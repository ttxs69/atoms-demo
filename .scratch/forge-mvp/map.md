...
- [对话 + 预览界面布局](issues/11-workspace-ui-layout.md) — **B 双栏分屏**为主形态（左对话右预览），
  ...
  **待实测风险**：E2B 预览 URL 能否被跨域 iframe 嵌入（文档未说明），
  这是 B 方案成立的前提，实现时第一件事就要验证
...
- **E2B 预览 URL 的 iframe 可嵌入性** —— ticket 11 的 B 方案（双栏分屏）
  以“预览能内嵌”为前提。E2B 文档未说明预览 URL 是否设了 `X-Frame-Options`
  或 CSP `frame-ancestors`。若不允许嵌入，B 方案不成立，退化为 A（抽屉）或新标签页。
  **实现时的第一个动作就是验证这个**
